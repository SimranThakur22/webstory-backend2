...
mongoose
  .connect(
    "mongodb+srv://simranthakur:Blueberry123@cluster0.mohhrpw.mongodb.net/webstory"
  )
  .then(() => {
...
app.get("/mystory", fetchUser, async (req, res) => {
  try {
    ...
    const story = await Story.distinct("creation_time", {
      user_id: req.user.id,
    });
...
app.get("/getmovies", async (req, res) => {
  try {
    const moviestories = await Story.find()
      .where("category")
      .equals("movie")
      .where("created_by")
      .equals("admin");
...
app.get("/getfood", async (req, res) => {
  try {
    const foodstories = await Story.find()
      .where("category")
      .equals("food")
      .where("created_by")
      .equals("admin");
...
app.get("/geteducation", async (req, res) => {
  try {
    const educationstories = await Story.find()
      .where("category")
      .equals("education")
      .where("created_by")
      .equals("admin");
...
app.get("/gethealth", async (req, res) => {
  try {
    const healthstories = await Story.find()
      .where("category")
      .equals("health and fitness")
      .where("created_by")
      .equals("admin");
    res.status(201).json({
      status: "success",
...
app.get("/gettravel", async (req, res) => {
  try {
    const travelstories = await Story.find()
      .where("category")
      .equals("travel")
      .where("created_by")
      .equals("admin");
...
app.get("/getsamecategorystory", async (req, res) => {
  try {
    const story = await Story.find()
      ...
      .where("creation_time")
      .equals(req.query.creation_time);
    res.status(201).json({
      status: "success",
      result: story.length,
      data: {
...
app.get("/top5stories", async (req, res) => {
  try {
    const topstories = await Story.distinct("creation_time", {
  ...
  try {
    ...
    if (user) {
      ...
      if (
        ...
        entered_username === user.username
      ) {
        const data = {
...
app.post("/addbookmark", fetchUser, async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate(
      { id: req.user.id },
      { bookmarkData: user.bookmarkData }
    );
...
app.post("/addlikestory", fetchUser, async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate(
      { id: req.user.id },
      { likedStoryData: user.likedStoryData }
    );
